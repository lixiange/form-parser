...
<template>
  ...
  <div
    class="banner"
    v-else-if="record.type === 'banner'"
    :style="`width:${
      record.options.alignMent === 'full' ||
      record.options.alignMent === 'align'
        ? '100%'
        : record.options.width
    };margin:${record.options.topVertical}px 0 ${record.options.bottomVertical}px`"
  >
    <div style="text-align: center" v-if="!record.options.bannerUrl">
      请上传图片
    </div>
    <img
      :style="`width:${
        record.options.alignMent === 'full' ? '100vw' : '100%'
      };flex-shrink:0`"
      v-else
      :src="record.options.bannerUrl"
    />
  </div>
  <!-- 自定义组件 -->
  <customComponent
